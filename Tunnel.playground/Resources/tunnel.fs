...
uniform float time;
uniform vec2 resolution;        // view port's dimension
uniform float factor;           // fade factor
...
// gl_FragCoord is the current fragment's window relative position (x, y, z, 1/w).
// The depth of the fragment is given by gl_FragCoord.z.
void main(void)
{
    // p is [-1.0, 1.0]
    ...
    fragColor = mix(vec4(0.0), t(uv)*r*r*2.0,
                    factor);
}
