...
/*
 ...
 The function compileProgram should be used to create separate
 program objects which can be combined to produce a program pipeline.
*/
import Cocoa
import OpenGL.GL3
...
public class GLShader
{
	...
	*/
    ...
    //==================================================================
    // This function should be called to instantiate separate program objects
    // which can be combined to produce a program pipeline.
    public func compileProgram(_ filename: String, shaderType: GLenum) ->GLuint {
        let mainBundle = Bundle.main
        let comps = filename.components(separatedBy:".")
        //print("\(comps)")
        let shaderPath: String? = mainBundle.path(forResource: comps[0],
        ...
        let contents = source!.cString(using: String.Encoding.utf8)	// [CChar]?
        var cStringPtr = UnsafePointer<GLchar>(contents)
        let programName = glCreateShaderProgramv(shaderType, 1, &cStringPtr)
        var infoLogLength: GLint = 0
        glGetProgramiv(programName, GLenum(GL_INFO_LOG_LENGTH), &infoLogLength)
        if infoLogLength > 0 {
            let infoLog = UnsafeMutablePointer<CChar>.allocate(capacity: Int(infoLogLength))
            glGetProgramInfoLog (programName, infoLogLength, nil, infoLog);
            let errMsg = String(utf8String: infoLog)
